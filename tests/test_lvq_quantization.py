...
def test_reconstruction_beats_trivial():
    ...
    q.fit(X)
    xhat = q.decompress(q.compress(X))
    assert float(np.mean((X - xhat) ** 2)) < float(np.var(X))
...
def test_constant_residual_row_is_safe():
    # a vector equal to the mean has zero residual -> delta guard must not blow up
    X = _data()
    q = LVQQuantizer(num_bits=4)
    q.fit(X)
    const = q.mu.reshape(1, -1).astype(np.float32)
    codes = q.compress(const)
    xhat = q.decompress(codes)
    assert np.isfinite(xhat).all()
